...
gapfill_data_m <- function (x, ndim = 10) {
  ...
  bpca_res <- pcaMethods::pca(x, "ppca", nPcs = ndim, completeObs = TRUE)

  bpca_res@completeObs
}
...
dimred_pw_iso <- function(x, knn = get_knn(x), ciso = FALSE,
  ...
  xd <- pw_isomap_holes(x, knn = knn, out_dim = 40, ciso = ciso,
...
assemble_correlations_m <- function(x) {
  
  n_var <- length(RELATIVE_INDICATORS)
  dim_N_iso<-x[[1]]$cor_iso %>% ncol()
  dim_N_PC<-x[[1]]$cor_pca %>% ncol()
  n_sam <- length(x)
  
  
  
  res_iso <- array(
    NA_real_, dim = c(n_var, dim_N_iso, n_sam),
    dimnames = list(RELATIVE_INDICATORS, paste0("iso", seq_len(dim_N_iso)), 1:n_sam)
  )
  res_pca <- array(
    NA_real_, dim = c(n_var, dim_N_PC, n_sam),
    dimnames = list(RELATIVE_INDICATORS, paste0("PC", seq_len(dim_N_PC)), 1:n_sam)
  )
...
make_qual_comp_res_inner <- function (i) {
  tryCatch({
    ...
    dta2     <- try( gapfill_data_m(dta$scaled_matrix, ndim = 20)    )
...
make_qual_comp_res <- function (x, data, data_sensitivity,
  ...
  parallel::clusterExport(cl, "gapfill_data_m")
...
make_cons_iso_pca_m <- function (qual_comp_res,
                               wdi_data,
                               ...
                               ciso = TRUE) {
  #stopifnot(ncol(wdi_data_cons_gf)>2)
  ...
  wdi_data_cons_scaled <- scale(as.matrix(wdi_data_cons[, -1:-2]))
  wdi_data_cons_gf <- gapfill_data_m(wdi_data_cons_scaled, ndim = 80)
...
make_wdi_data_cons_df <- function(knn = 50,
  ...
  wdi_data_cons_df <- data.table(
    ...
    wdi_data_cons_pca$dim_red@data@data[, 1:50],
    check.names = FALSE
  )
...
pw_isomap_holes <- function(data, knn, out_dim = 2, id_cols = character(0),
  ...
  return(list(y = y, d_geo = dgx, eigen_vals = e$values))
}
...
make_qual_comp_res_score1 <- function (x, data, data_sensitivity,
                                .cluster = CLUSTER) {
  message("make_qual_comp_res")
  message(Sys.time(), ": set up cluster")
  ...
  parallel::clusterExport(cl, "COUNTRY_CODES")
  parallel::clusterExport(cl, "RELATIVE_INDICATORS")
  ord_idx <- order(data_sensitivity$score_1, decreasing = TRUE)
  parallel::clusterExport(cl, "ord_idx", envir = environment())
...
make_qual_comp_res_score3 <- function (x, data, data_sensitivity,
                                       .cluster = CLUSTER) {
  message("make_qual_comp_res")
  message(Sys.time(), ": set up cluster")
  ...
  parallel::clusterExport(cl, "COUNTRY_CODES")
  parallel::clusterExport(cl, "RELATIVE_INDICATORS")
  ord_idx <- order(data_sensitivity$score_3
                  , decreasing = TRUE)
  parallel::clusterExport(cl, "ord_idx", envir = environment())
