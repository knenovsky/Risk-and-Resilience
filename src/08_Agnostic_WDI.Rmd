...
pcaC<-fread("../data/08_wdi_pca_correlation.csv")
```
...
# Create train/test split for time series data
Xgboostinput_filtered$regionsval<-Xgboostinput_filtered$Regions %>% as.factor() 
# Convert the folds to indices for trainControl


# Create stratified folds based on the 'region' variable.
set.seed(123)  # For reproducibility
folds <- createFolds(Xgboostinput_filtered$regionsval, k = 8, list = TRUE)
# 
# custom_control <- trainControl(
#   method = "cv",
#   index = folds, 
...
predictedValue<-postResample(
      predict(finalmodel_iso, 
          Xgboostinput_filtered %>% dplyr::select(any_of(features))),  
        test_data %>% pull(Label)
      )
predictedValue
xgboost::xgb.ggplot.shap.summary(data = as.matrix(train_data %>% 
...
Label<-"Excess Mortality"
 ...
 features<-c("PC1","PC2","PC10","PC5","PC6","New cases")
 y_var <-  Label
dataX <- as.matrix(Xgboostinput_Input_dep %>% select(features))
# hyperparameter tuning results
param_list <- list(objective = "reg:squarederror",  # For regression
...
dimred2021_knn250_2 %>%  filter(year%in%2020) %>% select(`Country Code`,NY.GDP.PCAP.CD) %>%  filter(!is.na(NY.GDP.PCAP.CD)) %>% merge( . ,Xgboostinput_Input_dep_pca,by="Country Code") %>% rename(`GDP per Capita PPP`=NY.GDP.PCAP.CD)-> Xgboostinput_Input_dep_pca_a


plot_iso<-cbind(plot_data %>% arrange(ID),a=Xgboostinput_Input_dep_pca$`Country Code`,b=Xgboostinput_Input_dep_pca$`Month in pandemic`,c=Xgboostinput_Input_dep_pca$Regions,d=Xgboostinput_Input_dep_pca_a$`GDP per Capita PPP`)


#plot_iso2<-plot_iso %>%  arrange(d) %>%  mutate(sorted_id=1:n()) 
plot_iso2<-plot_iso %>%  arrange(desc(d),a,b) %>%  mutate(sorted_id=1:n()) 
plot_iso3<- plot_iso2 %>%  select(-a,-b,-c,-d)
#pcaMeanshap<-plot_iso3 %>% arrange(ID)%>% select(-ID,-group,-sorted_id) %>%  apply(  . ,1, function(x)sum(abs(x)))
# you may choose to zoom in at a location, and set y-axis limit using `y_parent_limit`
col_pal <- c("slategrey","#1F968B",  "#7570b3", "#e7298a", "#3CBB75","yellow3", "orange","slategrey")
pcaPLot<-shap.plot.force_plot(plot_iso3, zoom_in_location = 1, y_parent_limit = c(-20,50))+ ggtitle("Ordered by GDP per Capita") +theme(legend.position = "bottom") + scale_fill_viridis(discrete = T)
pcaPLot

#ashap
#shap.plot.force_plot_bygroup(plot_data)
#ggsave(pcaPLot,file="../figures/08_Shap_force_plot_per_capita_pca.png",width = 7,height = 5)
```
...
Label<-"Excess Mortality"
 features<-c("iso1","iso2","iso3","iso4","New cases")
 features<-c("PC1","PC2","PC10","PC6","PC12","New cases")
 y_var <-  Label
dataX <- as.matrix(Xgboostinput_Input_dep %>% select(features))
# hyperparameter tuning results
param_list <- list(objective = "reg:squarederror",  # For regression
...
#plot_iso2<-plot_iso %>%  arrange(d) %>%  mutate(sorted_id=1:n()) 
plot_iso2<-plot_iso %>%  arrange(desc(d),a,c) %>%  mutate(sorted_id=1:n()) 
plot_iso3<- plot_iso2 %>%  select(-a,-b,-c,-d)
pcaMeanshap<-plot_iso3 %>% arrange(ID)%>% select(-ID,-group,-sorted_id) %>%  apply(  . ,1, function(x)sum(abs(x)))
# you may choose to zoom in at a location, and set y-axis limit using `y_parent_limit`
...
# you may choose to zoom in at a location, and set y-axis limit using `y_parent_limit`
isoPLot<-shap.plot.force_plot(plot_iso3, zoom_in_location = 1, y_parent_limit = c(-20,50))+ ggtitle("Ordered by PC1") +theme(legend.position = "bottom")
isoPLot

# #ashap
# #shap.plot.force_plot_bygroup(plot_data)
# ggsave(isoPLot,file="../figures/08_Shap_force_plot_pca.png",width = 7,height = 5)
```
...
plot_iso3<- plot_iso2 %>%  select(-a,-b,-c,-d)
isoMeanshap<-plot_iso3 %>% arrange(ID)%>% select(-ID,-group,-sorted_id) %>%  apply(  . ,1, function(x)sum(abs(x)))
# you may choose to zoom in at a location, and set y-axis limit using `y_parent_limit`
# shap.plot.force_plot(plot_iso3, zoom_in_location = countryvalue, y_parent_limit = c(-20,50))
#shap.plot.force_plot_bygroup(plot_data)
plot_iso2$regions
# you may choose to zoom in at a location, and set y-axis limit using `y_parent_limit`
where<-plot_iso2 %>%  group_by(c) %>% summarise(regions=max(sorted_id))
isoPLot<-shap.plot.force_plot(plot_iso3, zoom_in_location = 2400, y_parent_limit = c(-20,50))+ ggtitle("ISO ordered by region") +theme(legend.position = "bottom") + geom_vline(xintercept =where$regions,color="grey30" ) + annotate("text", x = where$regions, y = 20, label = where$c, vjust = -1, hjust = 1)
shap.plot.force_plot_bygroup(plot_data)
```


```{r}
ggsave(ggarrange(pcaPLot,isoPLot,ncol = 1),file="../figures/08_Shap_forcebothiso.png",device = "png",width = 7,height =12)
```
...
```{r}
plot_iso2 %>% arrange(ID) %>% select(ID,a,b,c,d) %>% cbind(pcaMeanshap) %>% cbind(isoMeanshap)->thesevalues
thesevalues %>%  ggplot(aes(x=pcaMeanshap,isoMeanshap,color=c))+ geom_point() +facet_wrap(~c)+theme_bw() +scale_color_viridis()

plot(pcaMeanshap,isoMeanshap)
dimred2021_knn250_2 %>% select(`Country Code`,RegionSoviet) %>% unique() %>% mutate(Regions=as.numeric(as.factor(RegionSoviet))) %>% select(Regions,RegionSoviet) %>%  unique %>% arrange(Regions)
```
...
plot_iso %>%  filter(sorted_id>1350,sorted_id<1450)-> subsetISO4
 ...
 dimred2021_knn250_5 %>%  filter(`Country Code` %in% (subsetISO4$a %>%  unique)) %>% filter(year%in% 2020) %>%  select(1:4,RegionSoviet) %>%  unique %>% mutate(a=as.numeric(as.factor(RegionSoviet)))
```
