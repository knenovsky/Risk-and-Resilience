...
# D Visualization of Xgboost
Visualizing the results of an XGBoost model, especially when comparing latent space representations, can be approached in several ways:

### 1. Model Performance Metrics Visualization

A bar chart or dot chart can be useful in comparing performance metrics (like R-squared values, RMSE, etc.) across models.



### 5. Model Comparison

If you've multiple models, consider using a pairwise comparison like t-tests or Wilcoxon tests to check if the differences in performance metrics are statistically significant.

see results

### Notes:

- Replace placeholder code snippets with actual variables/data from your environment.
- Consider adding titles, axis labels, and legends for clarity.
- Different visualizations might be more appropriate based on the context and the audience. Adjust visualizations accordingly.

Lastly, always accompany visual representations with a narrative. State the findings clearly and elaborate on the significance of any differences observed.
#D complete model

rerun with extra and with region and with month
## wishes
EIgentlich sind die Fälle des letzten monats relevant für den folgemonat
##  PCA model
```{r}
Xgboostinput<- fread("../results/06_Dimred_Exmort__withpc_monthly.csv")
Xgboostinput<-Xgboostinput %>% mutate(yearmon=as.numeric(as.factor(str_remove(pattern = "[a-zA-Z_]+",identifier))),continent2=as.numeric(as.factor(continent)))
...
dim_features<-c("PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10","PC11","PC12","PC13","PC14","PC15","PC16","PC17","PC18","PC19","PC20","yearmon")
...
Xgboostinput_covar$continent2

Xgboostinput_covar2<-Xgboostinput_covar%>%select(any_of(c(feature_vector,dim_features,features1,"p_value_mort"))) %>%  drop_na()
...
Xgboostinput<-Xgboostinput_filtered2%>% scale() %>% data.table() %>% drop_na()
features<-c(features,"Country","MonthIndex","Regions") %>%  unique()
...
save(xgb_caret_iso,file = "../output/07_results3_iso_trained")
...
save(finalmodel_iso,file = "../output/07_results3_iso_trained")
load("../output/07_results3_iso_trained")
