...
tempyear %>%  filter(year%in%c(1900:2019)) %>% pivot_longer(names_to = "month",
               "Jan":"Dec",
               values_drop_na = TRUE) %>% mutate(month=case_when(month%in%"Jan"~1,
                                                                 ...
                                                                 month%in%"Oct"~10,
                                                                 month%in%"Nov"~11,
                                                                 month%in%"Dec"~12)) %>% group_by(ISO_code,month) %>% summarise(meanTemp=mean(value)) %>%  unique->  tempyear_mean


tempyear %>%  filter(year%in%c(2020)) %>% pivot_longer(names_to = "month",
               "Jan":"Dec",
               values_drop_na = TRUE) %>% mutate(month=case_when(month%in%"Jan"~1,
                                                                 month%in%"Feb"~2,
                                                                 month%in%"Mar"~3,
                                                                 ...
                                                                 month%in%"Oct"~10,
                                                                 month%in%"Nov"~11,
                                                                 month%in%"Dec"~12))-> tempyear_2020
tempyear %>%  filter(year%in%c(2021)) %>% pivot_longer(names_to = "month",
               "Jan":"Dec",
               values_drop_na = TRUE) %>% mutate(month=case_when(month%in%"Jan"~1,
                                                                 month%in%"Feb"~2,
                                                                 month%in%"Mar"~3,
                                                                 ...
                                                                 month%in%"Oct"~10,
                                                                 month%in%"Nov"~11,
                                                                 month%in%"Dec"~12))-> tempyear_2021


colnames(tempyear_mean)<-paste(colnames(tempyear_mean),"mean",sep="_")
cbind(tempyear_2020,tempyear_mean) %>%  mutate(diff_Temp=(value-meanTemp_mean)) %>% select(year,ISO_code,month,diff_Temp)->temp2020
cbind(tempyear_2021,tempyear_mean) %>%  mutate(diff_Temp=(value-meanTemp_mean)) %>% select(year,ISO_code,month,diff_Temp)->temp2021
rbind(temp2020,temp2021) %>% arrange(ISO_code,year)->out_temp
...
Xgboostinput<-out_temp  %>% mutate(isoyearmonth=paste0(ISO_code,"_",year,"_",month)) %>% rename(diffTemp=diff_Temp) %>%  select(isoyearmonth,diffTemp)%>% merge(Xgboostinput_second, . , by.x = "identifier",by.y = "isoyearmonth")
#Xgboostinput%>% fwrite("../results/06_Dimred_Exmort__withpc_monthly.csv")
```
...
```{r}
shap.plot.dependence(data_long = shap_long, x = 'iso4', y = 'new_cases_smoothed_per_million', color_feature = 'p_value_mort') + ggtitle("Age has a peak in the middle converning the mortality")
