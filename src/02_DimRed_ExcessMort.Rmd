...
isos2019_to2021<-wdi_data_cons_df[wdi_data_cons_df$year%in%2021,c("Country Code","P_mort_2021","iso1","iso2","iso3","iso4","iso5","iso6","iso7","iso8","iso9","iso10","year","pscore_mean","RegionSoviet")]
isos2019_to2020<-wdi_data_cons_df[wdi_data_cons_df$year%in%2020,c("Country Code","P_mort_2020","iso1","iso2","iso3","iso4","iso5","iso6","iso7","iso8","iso9","iso10","year","pscore_mean","RegionSoviet")]
...
isos2020_2021 %>%   na.omit()->isos2020_2021
  ...
  # Calculate the similarity matrix
cluster <- apcluster(negDistMat(r=2), scale(isos2020_2021[,c("Pmort","iso1","iso2","iso3","iso4")]) , q=0.0001) 
...
isos2020_2021 %>% filter(year%in% 2021) %>% 
  pivot_longer(names_to = "isos",
               "iso1":"iso4",
               values_drop_na = TRUE) %>% 
  ggplot(aes(x=scale(value),y=pscore_mean,color=as.factor(clusterid)))+ geom_point() +facet_wrap(~isos,scales = "free_x") +scale_color_brewer("Cluster",palette = "Dark2")+ggtitle("P Score Mortality[2n]")+ xlab("")+ theme_bw()+ theme(legend.position = "bottom") 
