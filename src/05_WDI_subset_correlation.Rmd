...
iso1_corr %>% filter(!is.na(iso1_all))%>%  
...
```{r}
seriescode
wdi_data_cons_df %>% ggplot(aes(x=iso4,y=SP.POP.7074.FE.5Y))+ geom_point()
```
...
```{r,eval=FALSE}
group_values<-wdi_data_cons_df$iso10 %>%  quantile()
thisiso<-paste0("iso10")
var_comb <- expand.grid(var_name = used_var_names,
                          iso_name = thisiso,
                          stringsAsFactors = FALSE)
