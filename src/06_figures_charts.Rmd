...
result_bootstrap <- foreach(icount(trials),
...
result_bootstrap_wdi_2021<-result_bootstrap


#save(result_bootstrap_wdi_2021,file = "../data/06_wdi_21_bootstrap_1000")
# load("../data/06_wdi_20_bootstrap_1000")
result_bootstrap_wdi_2021 %>% sapply( . , function(x)as.numeric(x)) %>%data.frame() %>%
  pivot_longer(names_to = "cooli",
               "RMSE":"explained_variance") %>%
  ggplot(aes(x=value))+ geom_density() +facet_wrap(~cooli,scales = "free")+ theme_tufte()
