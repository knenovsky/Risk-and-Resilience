...
##  a   Non Linear Estimation of Exmort
...
param_grid <- expand.grid(
  nrounds = c(50),
  max_depth = c(1, 3, 6),           # Maximum depth of a tree
  eta = c(0.05),
...
plotR<-rbind(a,b) %>% ggplot(aes(x=Rsquared,fill=Dataset))+geom_histogram(bins = 100)+theme_classic()+xlim(0,1)+scale_fill_manual(values = col_pal)
plotRMS<-rbind(a,b) %>% ggplot(aes(x=RMSE,fill=Dataset))+geom_histogram(bins = 100)+theme_classic()+xlim(5,15)+scale_fill_manual(values = col_pal)
g
```
...
system.time(result_bootstrap <- foreach(icount(trials),
    ...
    xgboost::xgb.ggplot.shap.summary(data = as.matrix(trainDAT %>% dplyr::select( -label)),model = finalmodel$finalModel)
...
result_bootstrap_wdi_2019toPScoreMean<-result_bootstrap


fwrite(result_bootstrap_wdi_2019toPScoreMean,file = "../results/06_wdi_19_to_pscoreMean_bootstrap_1000.csv")
# load("../data/06_wdi_20_bootstrap_1000")
IsoYear<-"2019"
Label<-"pscore_mean"
candidates<-c("iso1","iso2","iso3","iso4","iso5","iso6","iso7")

dimredData<-dimred2021_knn250_2[] %>% filter(year%in% 2019) %>% select(c(Label,candidates)) %>%  filter(!is.na(iso1),!is.na(pscore_mean))
...
system.time(result_bootstrap_dimred <- foreach(icount(trials),
    ...
    a<-xgboost::xgb.ggplot.shap.summary(data = as.matrix(trainDAT %>% dplyr::select( -label)),model = finalmodel$finalModel)
    thisone<-length(list.files("../test/dimred_bootstrap_rerun/"))+1
    importance_matrix %>% fwrite(paste0("../test/dimred_bootstrap_rerun/Dimred_run2019toPscoreMean_",thisone,".csv",sep=""))
    finalmodel$results
}
...
result_bootstrap_dimred_2021<-result_bootstrap_dimred
```
