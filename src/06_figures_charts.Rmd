...
max_res_var_1 <- 1
png("../figures/06_plots/pca_vs_iso.png",width = 720,height = 720,units = "px")
plot(pca_res[[1]],
...
# Plot the values
excessmonthy %>% filter(!is.na(p_value_mort)) %>%  mutate(`P-Score Excess Mortality`=case_when(p_value_mort>50~50,p_value_mort<(-15)~16,TRUE~p_value_mort)) %>% 
...
excessmonthy2<-merge(excessmonthy,dimredData,by="Country Code")
listofmonth<-excessmonthy$date %>%  unique
excessmonthy2 %>%  filter(date%in% listofmonth[1]) -> runthismonthlysubset
```
...
xgb_caret_dimred <- caret::train(x = runthismonthlysubset %>% dplyr::select(all_of(features)),
                   ...
                   method = "xgbTree",
                   verbosity = 0,trControl=trainControl(method="cv",
                                          number = 30),tuneGrid = param_grid
                   ,tuneLength = 1
